...
4. Merge steps 2-3 and rank by recency × engagement — this becomes `b2`. Keep the **recent** hotspots as-is — note the repo's credible angle **where one exists**, but relevance is not a filter (a builder wants to see what's hot, related or not). Per `b2` topic, write **one short paragraph + the source link**.
...
### b1. Builder digest
(X / Official blogs / Podcasts, full depth per builder-digest.md, every item with its link;
skip the whole subsection if the builder feeds were unreachable)
