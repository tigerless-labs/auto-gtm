...
- **Tone** — follow the shared contract [`../gtm-shared/references/tone.md`](../gtm-shared/references/tone.md), and **land on its voice-source gate before drafting**: read stored samples if present; otherwise **offer to capture the user's bloggers + own account first**; drop to the bundled exemplars only if the user skips, and say so in the draft. Your explicit ask governs content. This is an original post — no per-thread overlay. **Mimic cadence/structure, not opinions.**
