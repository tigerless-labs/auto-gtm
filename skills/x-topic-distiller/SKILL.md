...
Post sincerely, never engagement bait. Topics fall into two tracks, and **build-in-public is the primary one** — when both tracks yield topics, build-in-public leads.

### Track A — build-in-public (primary)

Progress on your own repo/product: version updates, new capabilities, big refactors, repositioning, decisions and their reasoning. The bar is **honesty and concreteness**, not a takeaway checklist:

- **Real** — something actually shipped, changed, or got decided in the session;
- **Plain** — what changed and what it enables, or the decision and its why. No hype, no vague "big things coming".

### Track B — value-share (secondary)

Sharing things that aren't yours (tools/people/insights) or standalone reflections. These have no build-in-public license, so they must hand the reader a concrete takeaway — at least one of:
...
4. **A lived observation elevated to insight** — a first-hand scene leading to a broader conclusion.

For Track A these four are a **bonus, not a gate** — pairing an update with a takeaway upgrades it from "update" to "strong post".
...
Reads `~/.claude/projects/<cwd-mapping>/*.jsonl` (per-project directories) by default, plus the repo's worktree/subdirectory session dirs (`<cwd-mapping>-…`, rows labeled `@<worktree>`; `--exact-cwd` opts out); `--all-projects` scans every project directory under `~/.claude/projects/` instead. Use it when the user wants topics from all their recent conversations, not just this repo's.
...
Distill from the session, Track A first:
- **Repo milestone extraction (Track A):** a major version-level change to the current repo that happened in the session (new capability, big refactor, repositioning, version release). Two valid framings: a **version update** (what changed, what it enables for the user), or — stronger — the change paired with **the reasoning behind it** and **the cognition it projects** ("we removed X, because we realized Y — which means Z").
- **Entity extraction (Track B):** tools/products/people mentioned in the conversation → candidate share subjects.
- **Insight/reflection extraction (Track B):** one takeaway that stands on its own, or one reflection worth sharing.

### 3. Value gate

**Not every conversation yields a postable topic.** Gate by track:

- **Track A** passes if the change is real and stated plainly (drop anything that's hype or too vague to state concretely).
- **Track B** passes only with one of the four reader-takeaways from the Stance.

If no candidate passes, **say plainly "this conversation has no topic worth posting" and stop** — don't force one.
...
- **Reflection-type** → anchor in something that actually happened in the session, then elevate (takeaway #4 in the Stance).
- **Track A topics take either kind**: share-type when framed as a version update (the entity is the repo itself), reflection-type when framed as decision → why → what it reveals.

## Output

Output format is not enforced. **List Track A topics first.** By default, give per topic: **a one-line topic angle + track (A/B) + kind (share/reflection) + the entity being shared (share-type only) + the reader takeaway (required for Track B; bonus for Track A) + why it's worth posting (1 line)**. Follow the user's format if they ask for another.
