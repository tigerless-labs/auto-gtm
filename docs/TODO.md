...
- **回复类的输出契约是报告形状**：`reddit-auto-comment-draft` 一次输出 5 线程 × 3 变体 = 15 个并排 block，格子之间会自我对齐——2 句的挨着 12 行的显得没写完，于是全体向长里收敛（实测：只有高赞回复全为一行的线程压过了这个压力）。`#55` 把样本源换成线程自身评论，修的是材料，没动这个形状压力。待评估：阶梯三档只在「提不提产品」一个轴上变化，差异化只能靠加内容；以及批量表格是否该改成逐条产出。
