...
**精选而非枚举,配额是生成前的前提**:两类各自封顶少量(上限是 skill 里的具名旋钮),先通读全部素材再只写入选项——不是先全量生成再截断,那样入选项只会是原列表的头几条,而非重新组织过的叙事。**落选的完全不出**,连名字都不列:用户要的是判断,不是菜单。取舍标准是**受众视角**——读者会因此改变认知或行动吗;完整性不是标准。**launch 是复合叙事**:把 repo 的几个主要点合成能独立成篇的整体,而非一点一条。
